...
if __name__ == "__main__":
    ...
    image_names = [os.path.basename(x) for x in glob.glob(args.img_dir + "*-image.png")]

    df = pd.DataFrame({"filename": image_names})
    df["detections"] = False

    for idx, img_name in df["filename"].items():
        image = cv2.imread(os.path.join(args.img_dir, img_name))
        # Some images in BBD (813 out of 15828) have black borders (which are as well reflected in the masks)
        # from the tiling process, remove these from the list.
        if ('bbd1k' in args.img_dir) and (np.any(np.all(image==0, axis=2))):
            df.drop(idx, inplace=True)
        else:
            mask = cv2.imread(os.path.join(args.img_dir, img_name[:-9] + 'osm.png'), 0).astype(np.uint8)
            if np.any(mask==0):
